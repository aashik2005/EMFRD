"""
Training script for RoBERTa + Contrastive Learning model
...
def train_epoch(model, dataloader, optimizer, scheduler, device, epoch):
    ...
    avg_con_loss = total_con_loss / len(dataloader)

    return avg_loss, avg_cls_loss, avg_con_loss
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            outputs = model(
                input_ids=input_ids,
                attention_mask=attention_mask,
                return_contrastive=False,  # Don't need contrastive during eval
            )
...
def main(args):
    ...
    print("=" * 80)
    print("EMFRD - RoBERTa + Contrastive Learning Training")
    print("=" * 80)
    ...
    if (splits_dir / "train.json").exists():
        ...
        train_data, val_data, test_data = splitter.load_splits(splits_dir)
    else:
        print("\nCreating new splits...")
        splitter = DataSplitter(
            train_ratio=settings.TRAIN_RATIO,
    ...
    for epoch in range(1, settings.MAX_EPOCHS + 1):
        ...
        # Train
        train_loss, cls_loss, con_loss = train_epoch(
            model, train_loader, optimizer, scheduler, device, epoch
        )
        print(f"Train Loss: {train_loss:.4f} (Cls: {cls_loss:.4f}, Con: {con_loss:.4f})")
        training_history["train_loss"].append(train_loss)
        training_history["train_cls_loss"].append(cls_loss)
        training_history["train_con_loss"].append(con_loss)
    ...
    print("\n" + "=" * 80)
    print("Comparison with Paper Reference")
    print("=" * 80)
    paper_reference = {
        "accuracy": 0.968,
        "precision": 0.961,
    ...
    MetricsCalculator.compare_with_paper(
        test_results, paper_reference, "RoBERTa + Contrastive"
    )
    ...
    results_file = results_dir / f"roberta_contrastive_{timestamp}.json"

    experiment_results = {
        "experiment_id": f"roberta_contrastive_{timestamp}",
        "model": "roberta_contrastive",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train RoBERTa + Contrastive")
